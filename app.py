...
with app.app_context():
    ...
    if not Usuario.query.filter_by(username='admin').first():
        db.session.add(Usuario(username='admin', password=generate_password_hash('admin123', method='pbkdf2:sha256'), nombre='Admin', apellido='General', rol='admin'))
        db.session.commit()
...
def registro():
    if request.method == 'POST':
        ...
        nuevo = Usuario(
            username=request.form['username'], password=generate_password_hash(request.form['password'], method='pbkdf2:sha256'),
            nombre=request.form['nombre'], apellido=request.form['apellido'], email=request.form['email'], telefono=request.form['telefono'], rol='alumno'
        )
...
def obtener_dias_semana_actual():
    ...
    return [{'nombre': n, 'fecha_obj': inicio_semana + timedelta(days=i), 'fecha_str': (inicio_semana + timedelta(days=i)).strftime('%Y-%m-%d')} for i, n in enumerate(['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'])]
...
def admin_panel():
    ...
    activos = len([u for u in alumnos if u.disciplina_asignada and u.vencimiento and u.vencimiento >= hoy])
    pendientes = len(alumnos) - activos
    clases_hoy = Clase.query.filter_by(fecha=hoy.strftime('%Y-%m-%d')).all()
    reservas_hoy = sum([len(c.reservas) for c in clases_hoy])
    
    return render_template('admin.html', usuarios=alumnos, activos=activos, pendientes=pendientes, clases_hoy=len(clases_hoy), reservas_hoy=reservas_hoy)
...
def admin_asignar(usuario_id):
    ...
    if request.method == 'POST':
        ...
        usuario.vencimiento = date.today() + timedelta(days=int(request.form['dias']))
        db.session.commit()
        flash('Actividad y plan asignados.', 'success')
...
def admin_clases():
    ...
    if request.method == 'POST':
        ...
        fecha_clase = inicio_semana + timedelta(days=int(request.form['dia_semana']))
        for _ in range(int(request.form['meses']) * 4):
            fecha_str = fecha_clase.strftime('%Y-%m-%d')
            if not Clase.query.filter_by(disciplina_id=request.form['disciplina_id'], fecha=fecha_str, hora=request.form['hora']).first():
                db.session.add(Clase(disciplina_id=request.form['disciplina_id'], fecha=fecha_str, hora=request.form['hora'], cupo=request.form['cupo']))
            fecha_clase += timedelta(days=7)
        db.session.commit()
        flash('Turnos generados.', 'success')
        return redirect(url_for('admin_clases'))
        
    dias = obtener_dias_semana_actual()
    clases = Clase.query.filter(Clase.fecha >= dias[0]['fecha_str'], Clase.fecha <= dias[-1]['fecha_str']).order_by(Clase.fecha, Clase.hora).all()
    return render_template('admin_clases.html', clases=clases, disciplinas=Disciplina.query.all(), dias_semana=dias)
...
def alumno_panel():
    ...
    dias_restantes = (current_user.vencimiento - date.today()).days if current_user.vencimiento else -1
    dias_semana = obtener_dias_semana_actual()
    clases = Clase.query.filter(Clase.fecha >= dias_semana[0]['fecha_str'], Clase.fecha <= dias_semana[-1]['fecha_str'], Clase.disciplina_id == current_user.disciplina_asignada_id).order_by(Clase.fecha, Clase.hora).all()
    return render_template('alumno.html', clases=clases, dias_semana=dias_semana, reservas_dict={r.clase_id: r.id for r in current_user.mis_reservas}, dias_restantes=dias_restantes)
...
def cancelar_reserva(reserva_id):
    ...
    if datetime.strptime(f"{reserva.clase_reservada.fecha} {reserva.clase_reservada.hora}", "%Y-%m-%d %H:%M") - datetime.now() < timedelta(minutes=30):
